...
截至 `2026-05-11`，月度选股已有三个 promoted 配置，当前 active 为 M8+quality indcap3：

- `active_promoted_config_id` 为 `monthly_selection_m8_indcap3_plus_quality`。
- 默认方法为 `U1_liquid_tradable + Top20 + M8_regime_aware_fixed_policy__indcap3 + quality`。
- 选择层执行 hard-cap 行业只数约束：`selection_policy=industry_names_cap`，`max_industry_names=3`，`cost_bps=10`。
- 该默认切换是 2026-05-11 owner override：不等待 2026-05-29 的 OOS #2，先使用当前回测最优方法。
