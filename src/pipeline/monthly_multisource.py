...
def build_feature_specs(
    ...
    *,
    ...
    family_cols: dict[str, tuple[str, ...]] = {
        ...
        "northbound_regime": tuple(NORTHBOUND_REGIME_RAW_FEATURES),  # 市场级特征不截面 z-score
        "margin_trading": tuple(f"{c}_z" for c in MARGIN_TRADING_RAW_FEATURES),
        "concept": tuple(f"{c}_z" for c in CONCEPT_RAW_FEATURES),
        "lhb": tuple(f"{c}_z" for c in LHB_RAW_FEATURES),
        "quality": tuple(f"{c}_z" for c in QUALITY_RAW_FEATURES),
        "reversal_volume": tuple(f"{c}_z" for c in REVERSAL_VOLUME_RAW_FEATURES),
        "liquidity_position": tuple(f"{c}_z" for c in LIQUIDITY_POSITION_RAW_FEATURES),
        "event": tuple(f"{c}_z" for c in EVENT_RAW_FEATURES),
    }
    family_order = [
        "industry_breadth",
        "fund_flow",
