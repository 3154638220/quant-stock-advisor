...
_FAMILY_FEATURE_PREFIX: dict[str, str] = {
    ...
    "northbound_regime": "feature_north_",
...
class M5RunConfig:
    ...
    # W5/I2: apply persisted weak-factor governance before feature specs are built.
    enforce_factor_governance: bool = True
    factor_governance_log: str | Path | None = None
...
_IC_DECAY_CHECK_FEATURES: tuple[str, ...] = (
    ...
    + LHB_RAW_FEATURES
    + QUALITY_RAW_FEATURES
    + REVERSAL_VOLUME_RAW_FEATURES
    + LIQUIDITY_POSITION_RAW_FEATURES
    + EVENT_RAW_FEATURES
)
...
def build_feature_specs(
    ...
    *,
    ...
    cumulative: list[str] = list(price_volume_cols)
    family_cols: dict[str, tuple[str, ...]] = {
        "industry_breadth": tuple(get_factor_cols("industry_breadth", use_zscore=True, only_active=True)),
        "fund_flow": tuple(get_factor_cols("fund_flow", use_zscore=True, only_active=True)),
        "fundamental": tuple(
            get_factor_cols(
                "fundamental",
                use_zscore=not use_industry_neutral_zscore,
                use_ind_zscore=use_industry_neutral_zscore,
                only_active=True,
            )
        ),
        "shareholder": tuple(get_factor_cols("shareholder", use_zscore=True, only_active=True)),
        "northbound": tuple(get_factor_cols("northbound", use_zscore=True, only_active=True)),
        "northbound_regime": tuple(get_factor_cols("northbound_regime", use_zscore=False, only_active=True)),
        "margin_trading": tuple(get_factor_cols("margin_trading", use_zscore=True, only_active=True)),
        "concept": tuple(get_factor_cols("concept", use_zscore=True, only_active=True)),
...
def _apply_factor_governance_if_configured(cfg: M5RunConfig) -> None:
    if not getattr(cfg, "enforce_factor_governance", True):
        return
    raw_path = getattr(cfg, "factor_governance_log", None)
    log_path = Path(raw_path) if raw_path else DEFAULT_FACTOR_GOVERNANCE_LOG
    apply_factor_governance_log(log_path)


def _drop_inactive_factor_columns(dataset: pd.DataFrame, enabled_families: list[str]) -> pd.DataFrame:
    families = set(enabled_families) | {"price_volume"}
...
def attach_enabled_families(
    ...
    _apply_factor_governance_if_configured(cfg)
    ...
    out = _drop_inactive_factor_columns(out, enabled_families)
