...
从 build_industry_map 脚本迁入 src/ 的核心逻辑：
- 行业映射拉取（东方财富 / 申万）
- symbol 规范化与去重
- universe 对齐
- 质量摘要

仅包含算法逻辑，不含 CLI 参数解析与文件输出编排。
"""
...
def load_current_universe(
    *, duckdb_path: str | Path, universe_json: str | Path
) -> tuple[list[str], str]:
    db = Path(duckdb_path)
    if db.exists():
        try:
...
def fetch_sw_industry_mapping(
    ...
    third = _call_with_retry(
        lambda: ak.sw_index_third_info(),
        retries=retries,
        delay_sec=retry_delay_sec,
    )
    second = _call_with_retry(
        lambda: ak.sw_index_second_info(),
        retries=retries,
        delay_sec=retry_delay_sec,
    )
    second_to_first = {}
    ...
    if not all_rows:
        return pd.DataFrame(columns=REQUIRED_COLUMNS)
    return pd.DataFrame(all_rows)[REQUIRED_COLUMNS]
    ...
    *,
    ...
    if source_norm == "eastmoney":
        return fetch_industry_mapping(
            sleep_sec=sleep_sec, asof_date=asof_date,
            retries=retries, retry_delay_sec=retry_delay_sec,
        )
    if source_norm in {"sw", "shenwan"}:
        return fetch_sw_industry_mapping(
            sleep_sec=sleep_sec, asof_date=asof_date,
            retries=retries, retry_delay_sec=retry_delay_sec,
            request_timeout_sec=request_timeout_sec, max_workers=max_workers,
        )
    if source_norm != "auto":
...
def align_to_universe(
    mapping: pd.DataFrame, universe: list[str], asof_date: str
) -> pd.DataFrame:
    """将映射与 universe 对齐：missing 标的标记为 unknown。"""
    if not universe:
        return mapping.copy()
...
def quality_summary(
    mapping: pd.DataFrame, universe: list[str], universe_source: str, asof_date: str
) -> IndustryMapQuality:
    ...
    known = (
        in_universe["industry"].astype(str).str.strip().ne("unknown")
        & in_universe["industry"].astype(str).str.strip().ne("")
    )
...
def build_quality_doc(
    quality: IndustryMapQuality, industry_sizes: pd.DataFrame, output_csv: Path, project_root: Path
) -> str:
    row = quality.to_row()
    lines = [
