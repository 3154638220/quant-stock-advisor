...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| G0 | -38.57% | -2.96% | +7.24% | +6.22% | -36.63% | -16.69% |
| G1 | -42.45% | -34.57% | -14.13% | -46.15% | -42.46% | -5.40% |

## 结论

本轮不改变默认研究基线，也不允许 promotion。

`market_relative` 标签在 light proxy 上看似改善 G1，但正式回测完全不成立，尤其 `2022/2024/2025` 明显恶化。P1 的问题不只是“标签是否相对市场”，而是训练期 proxy Top-K 与正式月频换手/执行成本/全样本时段之间仍有断层。

下一步应优先做两件小而硬的诊断：

1. 对 `rank + market_relative` 的 G1 做 rank bucket 与持仓暴露拆解，解释 light proxy 到 full backtest 的翻车点。
2. 对 `regression + rank_fusion` 的 G0 补一次 full backtest；它在 light proxy 中超过 rank 目标，但不应在未正式验证前推进。
