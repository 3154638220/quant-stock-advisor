...
from src.data_fetcher.margin_trading_client import run_margin_trading_quality_checks
    ...
    for _, row in report_df.iterrows():
        ...
        if family == "fund_flow":
            ...
            if ok:
                ...
                decision = "暂停新增研究预算，仅保留数据链路维护。"
                reason = notes or "基础质量检查未通过。"
        elif family == "margin_trading":
            if ok:
                decision = "保留高优先级研究预算，推进 M5 增量 delta 测试。"
                reason = "融资融券历史长、覆盖广、与现有因子相关性低，增量潜力大；M1 质量诊断通过，进入 M5 增量。"
            else:
                decision = "暂停研究预算，优先修复数据质量/覆盖率问题。"
                reason = notes or "融资融券 M1 质量诊断未通过。"
        else:
            decision = "暂不投入研究预算。"
...
def _build_alignment_breakdown(report_df: pd.DataFrame) -> str:
    ...
    margin = report_df[report_df["family"] == "margin_trading"]
    if not margin.empty:
        row = margin.iloc[0]
        max_d = str(row.get("max_date", "") or "")
        distinct_sym = int(row.get("distinct_symbols", 0) or 0)
        coverage = row.get("coverage_ratio_vs_daily", None)
        cov_str = f"{float(coverage):.1%}" if coverage is not None and not pd.isna(coverage) else "未知"
        lines.append(
            f"- `margin_trading`：最新 `{max_d}`，"
            f"覆盖 `{distinct_sym}` 只标，日线覆盖率 `{cov_str}`。"
        )
...
def main() -> int:
    ...
    allowed = {"fund_flow", "shareholder", "margin_trading"}
    ...
    with duckdb.connect(db_path, read_only=True) as con:
        ...
        if "margin_trading" in families:
            margin_report = run_margin_trading_quality_checks(
                con,
                table=f"{args.flow_table}_margin" if "margin" not in args.flow_table else "a_share_margin_trading",
                daily_table=args.daily_table,
            )
            # Use a dedicated arg or fall back to default table name
            if "margin" not in args.flow_table:
                # Re-run with correct table name
                margin_report = run_margin_trading_quality_checks(
                    con,
                    table="a_share_margin_trading",
                    daily_table=args.daily_table,
                )
            payload["margin_trading"] = {
                "ok": margin_report.ok,
                "table_exists": margin_report.table_exists,
                "total_rows": margin_report.total_rows,
                "distinct_symbols": margin_report.distinct_symbols,
                "max_date": margin_report.max_trade_date,
                "min_date": margin_report.min_trade_date,
                "duplicate_pk_rows": margin_report.duplicate_pk_rows,
                "coverage_ratio_vs_daily": margin_report.coverage_ratio_vs_daily,
                "daily_max_date": margin_report.daily_max_trade_date,
                "rows_without_daily_match": margin_report.rows_without_daily_match,
                "median_symbols_per_day": margin_report.median_symbols_per_day,
                "notes": " | ".join(margin_report.notes),
            }
            report_rows.append(
                {
                    "result_type": "newdata_quality_summary",
