...
默认先请求 AkShare 全市场列表（与 fetch_only 同源）；失败时可用 --use-cache
仅依据 data/cache/universe_symbols.json 与库内做差集。
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--use-cache",
        action="store_true",
        help="不请求网络，仅用本地 universe_symbols.json 与库做差",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    in_db = set(_list_symbols_from_duckdb(config_path=args.config))
    log.info("DuckDB  distinct symbols: %s", len(in_db))

    universe: list[str] = []
    source = ""
    if args.use_cache:
        universe = _load_symbols_from_local_cache(config_path=args.config)
        source = "local_universe_cache"
    else:
        codes = _fetch_akshare_universe_codes(
            config_path=args.config,
            source_timeout_sec=float(ak.get("universe_source_timeout_sec", 60.0)),
            source_retries=max(1, int(ak.get("universe_source_retries", 2))),
            retry_delay_sec=float(ak.get("universe_retry_delay_sec", 3.0)),
        )
        if codes:
            universe = codes
