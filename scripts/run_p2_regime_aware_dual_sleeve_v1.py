...
def main() -> None:
    ...
    identity = make_research_identity(
        result_type="p2_regime_aware_dual_sleeve",
        research_topic="p2_regime_aware_dual_sleeve",
        research_config_id=f"p2_dual_sleeve_{slugify_token(prefix)}",
        output_stem=prefix,
    )
    data_slice = DataSlice(
        dataset_name="p2_regime_aware_dual_sleeve_backtest",
        ...
        feature_set_id="p2_dual_sleeve_factors",
        feature_columns=(),
        label_columns=(),
        ...
        extra={
            ...
            "defensive_sleeve": "vol_to_turnover",
            "upside_sleeve": "limit_up_hits_20d + tail_strength_20d",
            "sleeve_rules_count": int(len(SLEEVE_RULES)),
        },
    )
    artifact_refs = (
    ...
    top_leader = leaderboard.sort_values("daily_bt_like_proxy_annualized_excess_vs_market", ascending=False).head(1)
    metrics = {
        "candidate_count": int(len(leaderboard)),
        "accept_count": int(sum(1 for v in accept_map.values() if v == "accept")),
        "gray_zone_count": int(sum(1 for v in accept_map.values() if v == "gray_zone")),
        "reject_count": int(sum(1 for v in accept_map.values() if v == "reject")),
    }
    if not top_leader.empty:
        metrics["top_daily_proxy_excess"] = float(top_leader.iloc[0]["daily_bt_like_proxy_annualized_excess_vs_market"])
    gates = {
        "data_gate": {"passed": bool(len(leaderboard) > 0), "candidate_count": int(len(leaderboard))},
    ...
    result = ExperimentResult(
        ...
        promotion={
            ...
            "blocking_reasons": ["p2_dual_sleeve_is_historical_research_only"],
        },
        notes="Historical P2 regime-aware dual sleeve experiment; not a promotion candidate.",
    )
